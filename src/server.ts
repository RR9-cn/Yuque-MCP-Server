    ...
    this.server.tool(
      ...
      async ({
        ...
        try {
          ...
          const doc = await this.yuqueService.createDoc(
            namespace,
            title,
            slug,
            body,
    ...
    this.server.tool(
      ...
      async ({
        ...
        try {
          ...
          const doc = await this.yuqueService.updateDoc(
            namespace,
            id,
            updateData
          );
    ...
    this.server.tool(
      ...
      async ({ query, type, scope, page, creator }) => {
        try {
          ...
          const results = await this.yuqueService.search(
            query,
            type,
            scope,
            page,
    ...
    this.server.tool(
      ...
      async (params) => {
        try {
          ...
          const stats = await this.yuqueService.getGroupMemberStatistics(
            login,
            queryParams
          );
    ...
    this.server.tool(
      ...
      async (params) => {
        try {
          ...
          const stats = await this.yuqueService.getGroupBookStatistics(
            login,
            queryParams
          );
    ...
    this.server.tool(
      ...
      async (params) => {
        try {
          ...
          const stats = await this.yuqueService.getGroupDocStatistics(
            login,
            queryParams
          );
    ...
    this.server.tool(
      ...
      async ({ namespace }) => {
        try {
          ...
          return {
            content: [
              { type: "text", text: `Error fetching toc for repo: ${error}` },
            ],
          };
        }
      }
    ...
    this.server.tool(
      ...
      "更新文档目录，文章发布完成后，需更新文档目录",
      {
        ...
        action_mode: z
          .enum(["sibling", "child"])
          .optional()
          .describe("操作模式 (sibling:同级, child:子级)"),
        target_uuid: z
          .string()
          .optional()
          .describe(
            "目标节点 UUID, 必填，获取方式: 调用获取知识库目录接口获取"
          ),
        doc_ids: z
          .array(z.number())
          .optional()
          .describe("文档 ID 数组 [必填]"),
        node_uuid: z
          .string()
          .optional()
          .describe(
            "操作节点 UUID [移动/更新/删除必填]，获取方式: 调用获取知识库目录接口获取"
          ),
        type: z
          .enum(["DOC", "LINK", "TITLE"])
          .optional()
          .describe("节点类型， [创建必填] (DOC:文档, LINK:外链, TITLE:分组)"),
        title: z.string().optional().describe("节点名称 [创建分组/外链必填]"),
        url: z.string().optional().describe("节点外链 [创建外链必填]"),
        open_window: z
          .number()
          .optional()
          .describe("是否新窗口打开 [外链选填] (0:当前页打开, 1:新窗口打开)"),
        visible: z
          .number()
          .optional()
          .describe("节点是否可见 [创建必填] (0: 不可见, 1: 可见)"),
      },
      async (params) => {
        try {
          debugger;
          const { namespace, ...queryParams } = params;
          const stats = await this.yuqueService.updateRepoToc(
            namespace,
            queryParams
          );

          Logger.log(`Update repo toc for namespace: ${namespace}`);
          return {
            content: [{ type: "text", text: JSON.stringify(stats, null, 2) }],
          };
        } catch (error) {
          Logger.error(`Update repo toc for namespace: ${error}`);
          return {
            content: [
              {
                type: "text",
      ...
      if (queryAccessToken || queryBaseUrl) {
        console.log(
          `Using custom configuration: ${
            queryAccessToken ? "Token from query, " : ""
          }${queryBaseUrl ? "BaseUrl from query" : ""}`
        );
